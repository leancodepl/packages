...
class MockCaptureDevice: NSObject, FLTCaptureDevice {
  ...
  var setVideoZoomFactorStub: ((Float) -> Void)?
  ...
  func torchMode() -> AVCaptureDevice.TorchMode {
    return getTorchModeStub?() ?? AVCaptureDevice.TorchMode.off
  }

  func setTorchMode(_ torchMode: AVCaptureDevice.TorchMode) {
    setTorchModeStub?(torchMode)
  }
  ...
  func videoZoomFactor() -> Float {
    return 0
  }

  func setVideoZoomFactor(_ factor: Float) {
    setVideoZoomFactorStub?(factor)
  }
